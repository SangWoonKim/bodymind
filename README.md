...
- 3월 13일 기준
- 운동 기능 개발 1순위
- (운동 ui 제작 진행중, 운동ui 상세 진행 중 - 데이터 ui 간 바인딩 진행중)
- (이슈 - ExerciseClassify(ExerciseType간 매핑 필요), 사용자 몸무게 이력 테이블 생성 필요(bmr산출을 위해), ExerciseClassify에 따른 아이콘 및 색상표 정의)
- 수면 기능 개발 2순위
...
- 추후 체중 입력을 넣어 일자별로 bmr 산출하는 로직 조회 필요 후순위(운동에 대한 해당 운동날 bmr에 대한 데이터 산출을 위해 이력이 필요함)
